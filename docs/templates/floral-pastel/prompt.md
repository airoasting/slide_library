...
- 좋은 예: "꽃그림 파스텔", "오늘의 시작", "한 주 살펴보기", "오늘의 일정", "활동 구성", "중점 영역", "우리 팀", "진행 방식".
...
**이 템플릿에만 적용되는 예외**
- (규칙 6) 마무리를 §7 인용 레이아웃으로 차용할 때 attribution에 실존 인물 이름을 지어내지 않는다. 화자를 특정하지 않는 맥락 표기를 쓰고, 격려 한 줄과 연락처·일정 한 줄로 닫는다.
