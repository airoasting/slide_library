...
- 좋은 예 (평서문): "정밀히 살필수록 신호와 잡음의 경계가 드러납니다."
...
- 마무리 `.slide-closing`:
  ...
  - h1: 짧은 명제 또는 인사. 권고: 본 템플릿 기본 `감사합니다` 대신 덱의 결론을 담은 한 문장으로 교체. 일반론이 아니라 앞 슬라이드의 수치나 판단에서 나온 문장이어야 한다(예: `네 분기 연속 목표를 넘겼습니다.`).
  - `<p>`: `질의 및 토론` 또는 짧은 안내.
...
**이 템플릿에만 적용되는 예외**
- (규칙 1) 본 템플릿에는 별도 출처 라인 클래스가 없어 §4.4에 따라 출처를 본문 단 안에 문장으로 넣는다. 그 출처 문장은 캡션이 아니라 본문이므로 종결어미를 그대로 쓴다.
- (규칙 6) §4.6의 마무리 명제 예시 `절제된 선택이 결과를 만듭니다.`는 일반론이므로 카피로 쓰지 않는다. 덱의 결론 수치나 판단을 담은 한 문장으로 바꾼다.
