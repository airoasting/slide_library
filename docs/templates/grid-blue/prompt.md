...
- 좋은 예: "트렌드란 <span class='roman'>사람들이 서로 다른 공간에서 거의 같은 시기에 조용히 꺼내는 공통 질문입니다</span>", "소프트웨어는 하나의 방입니다. 그 방은 사람이 천천히 머물 수 있도록 설계합니다", "가을호에서 다시 만납니다".
...
1. **라벨에 종결어미를 붙이지 않는다.** 종결 체 통일은 본문 문장에만 적용한다. 라벨, 캡션, 스펙 항목, 목차 줄, 축 이름, 범례는 명사구로 끝낸다. "연락처입니다", "제1판입니다", "4개 부문 재무 성과, 정규화 지수입니다"는 라벨이 문장 흉내를 낸 자리다. 발표자가 소리 내어 읽을 수 없으면 라벨이 아니라 문장이 된 것이다.
2. **조사 없이 이어지는 명사는 3개까지.** 넘으면 절로 풀어 쓰거나 문장을 둘로 나눈다. "파트너와 채널, 사용자 10배 성장을 품질 저하 없이 감당할 운영 역량"처럼 명사를 쌓으면 주술 호응이 무너지고 한 호흡에 읽히지 않는다.
3. **사람이 하는 동사에는 사람이나 조직이 주어로 온다.** 찾다, 만들다, 이끌다, 키우다, 예측하다, 잡아내다 앞에 지표·시스템·분석을 주어로 세우지 않는다. "고급 분석이 패턴을 찾아내고 이상 징후를 잡아냅니다"가 아니라 "고급 분석으로 패턴과 이상 징후를 찾습니다".
...
6. **마지막 슬라이드는 사실, 날짜, 담당자, 다음 행동으로 닫는다.** 교훈이나 격언으로 닫지 않는다. "성장은 목적지가 아닌 과정입니다", "명확함에서 창의성이 나옵니다" 같은 문장은 앞의 구체적 항목과 논리로 이어지지 않고 장식으로 얹힌다. 인용을 넣을 때는 실제 출처와 화자를 확인해 적고, 확인할 수 없으면 넣지 않는다.
7. **`template.json`의 tone과 formality를 먼저 읽고 전 슬라이드를 그 한 목소리로 쓴다.** 손글씨 템플릿 본문에 사내 위키 문체가, 포스터 템플릿에 IR 보도자료 문체가 얹히면 한 덱을 두 사람이 쓴 것처럼 읽힌다. 껍데기가 정한 톤과 카피의 톤이 어긋나는 것을 독자는 번역투 한 줄보다 크게 느낀다.
8. **같은 서술어는 한 덱에서 4회를 넘기지 않는다.** 같은 문형은 연속 3항목까지만 쓰고, 병렬 목록은 마지막 항목에서 리듬을 한 번 바꾼다. 정확도는 유지되지만 사람이 말하는 소리가 아니라 도장 찍는 소리로 읽힌다.
9. **수량은 "명사 + 수" 순서로 쓴다.** "11개 중 6개 마일스톤"이 아니라 "마일스톤 11개 중 6개", "2건의 리스크 식별"이 아니라 "리스크 2건 식별". 앰퍼샌드(&)는 가운뎃점이나 "와/과"로 바꾸고, 외래어는 표기법을 따른다(컬렉티브, 내비게이션, 콘셉트).
