...
- 형식 (JetBrains Mono 24px letter-spacing 0.06em ink-2 uppercase): `FIG. 01 · 값은 플레이스홀더 · 출처: 샘플 데이터셋 / 내부 자료`
...
**이 템플릿에만 적용되는 예외**
- (규칙 1) §4.4의 `FIG.` 캡션은 명사구로 끝낸다. `FIG. 01 · 값은 플레이스홀더 · 출처: 샘플 데이터셋 / 내부 자료` 형태로 쓰고 종결어미를 붙이지 않는다.
