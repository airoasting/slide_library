...
손에 든 PDF나 파워포인트가 있으시다면, `slide-library` 기능이 그 자료의 색, 글꼴, 배치, 문장 규칙을 분석해 같은 디자인으로 새 슬라이드를 만들 수 있는 설계도를 정리해 드립니다. AI 비서에게 이렇게 말씀하시면 됩니다.

```
@내파일.pdf 이 디자인으로 슬라이드를 만들 수 있게 분석해줘
```

결과물은 그 자료의 디자인 규칙을 담은 문서 한 편입니다. 이후로는 "이 설계도대로 새 슬라이드 만들어줘" 한마디면 같은 톤을 반복해서 쓰실 수 있습니다. 자세한 내용은 [`SKILL.md`](./SKILL.md)에 있습니다.
...
### v2.1 (2026-07-04)
35개 템플릿의 예시 카피를 사람이 쓴 문장으로 다듬어 AI 특유의 번역투와 상투구를 걷어냈습니다. `slide-library` 스킬은 산출물의 §1~§10 골격을 명확히 정의하고 색·폰트 추출 방법까지 담아 완성도를 높였습니다.
...
### v1.0 (2026-05-15)
산출물 기본 형식을 HTML에서 PPTX로 바꿨고, 기존 슬라이드를 분석해 시스템 프롬프트를 역추출하는 `slide-library` 스킬을 추가했습니다. 인덱스 페이지는 레트로 카세트 톤으로 다시 디자인했고, 사이트 제목도 `AI ROASTING · Slide Library`로 정식화했습니다. 이 시점부터 라이브러리는 단순 갤러리를 넘어 생성 도구로 확장됐습니다.
