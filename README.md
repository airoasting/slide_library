...
### v2.1 (2026-07-04)
35개 템플릿의 예시 카피를 사람이 쓴 문장으로 다듬어 AI 특유의 번역투와 상투구를 걷어냈습니다. `slide-clone` 스킬은 산출물의 §1~§10 골격을 명확히 정의하고 색·폰트 추출 방법까지 담아 완성도를 높였습니다. 저장소는 스크린샷을 `assets` 아래로 옮기고 내부 문서를 정리해 구조를 단정하게 다듬었습니다. README는 개발 지식이 없는 비즈니스 리더도 바로 이해하도록 다시 썼고, 상단에는 대표 이미지를 넣었습니다. 갤러리 주소는 Vercel로 옮겼습니다.
